trigger CalculateContractedAmounts on OpportunityLineItem (before update) {
    ...
    if(Trigger.IsUpdate)
    {
        ...
        { 
            ...
            else if((lineItem.Quote_Type__c=='EDGECONNECT'|| lineItem.Quote_Type__c=='EC-SP-Perpetual' || lineItem.Quote_Type__c=='EC-SP-Term'|| lineItem.Quote_Type__c=='EC-SP-Metered'||lineItem.Quote_Type__c=='Service Provider')&& lineitem.Product_Family__c=='Product')
            {
                lineItem.EC_Hardware_Amount__c =totalPrice;
            }
            else if((lineItem.Quote_Type__c=='EDGECONNECT'|| lineItem.Quote_Type__c=='EC-SP-Term' || lineItem.Quote_Type__c=='EC-SP-Metered' || lineItem.Quote_Type__c=='Service Provider'|| lineItem.Quote_Type__c=='EC-SP-Perpetual')&& lineitem.Product_Family__c!='Product' && lineitem.Product_Family__c!='Virtual Image' )
            {
